...
export default function Nutrition() {
  const { nutrition, setNutrition } = useData();
  const [error, setError] = useState('');

  // Helper: check if nutrition data has at least one row with a valid date
  const hasValidNutrition = Array.isArray(nutrition) && nutrition.some(row => row && (row.Date || row[Object.keys(row)[0]]));
  ...
  const handleFileUpload = (e) => {
    ...
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete: (results) => {
        // Check for at least one row with a valid date
        const valid = Array.isArray(results.data) && results.data.some(row => row && (row.Date || row[Object.keys(row)[0]]));
        if (valid) {
    ...
    <div className="relative min-h-screen flex flex-col justify-center items-center bg-gray-50 overflow-hidden">
      ...
      <div className="z-10 w-full max-w-3xl mx-auto flex flex-col gap-8 items-center pt-24 pb-16">
        <div className="bg-white/95 rounded-2xl shadow-xl px-8 py-10 flex flex-col gap-6 w-full border border-gray-100">
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-6 text-gray-900 text-center">Nutrition Data</h1>
          <ChatWithData data={nutrition} section="nutrition" />
          {!hasValidNutrition && (
          ...
          {hasValidNutrition ? (
            <NutritionVisualizations data={nutrition} />
          ) : (
